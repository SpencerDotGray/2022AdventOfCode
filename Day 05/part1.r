...
while(length(line <- readLines(f, n=1)) > 0) {
    ...
    if (doneSetup == FALSE && substring(line, 2, 2) != "1") {
        ...
        for (i in 1 : stack_numbers[1]) {
            sourceLength <- nchar(stacks[stack_numbers[2]])
            destLength <- nchar(stacks[stack_numbers[3]])
            
            # Source get and remove
            moveStack = substring(stacks[stack_numbers[2]], sourceLength, sourceLength)
            stacks[stack_numbers[2]] <- substring(stacks[stack_numbers[2]], 1, sourceLength - 1)
